...
\centering
\begin{tikzpicture}
\node[anchor=south west,inner sep=0] at (0,0) {\includegraphics[width=\columnwidth]{phase_precession.png}};
\end{tikzpicture}
\caption{\textbf{Phase precession as FFT.} (A) Place cell firing (raster). (B) Phase vs. position. (C) Fourier reconstruction (error $<$5\%).}
...
\centering
\begin{tikzpicture}
\node[anchor=south west,inner sep=0] at (0,0) {\includegraphics[width=\columnwidth]{reconstruction.png}};
\end{tikzpicture}
\caption{\textbf{Trajectory reconstruction.} True (black) vs. reconstructed (red) path from 12 Fourier coefficients.}
\label{fig:recon}
\end{figure}
...
\section{Methods}
Available at \url{https://github.com/PaulWolfCO/fourier-memory}.
